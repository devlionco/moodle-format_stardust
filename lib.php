...
define('FORMAT_STARDUST_COLLAPSED', 0);
define('FORMAT_STARDUST_EXPANDED', 1);
define('FORMAT_STARDUST_PINNED', 1);
define('FORMAT_STARDUST_UNPINNED', 0);
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_stardust extends format_base {
    /**
    ...
    protected function find_collapsed_parent($section, $returnid = false) {
        $section = $this->get_section($section);
        if (!$section->section || $section->collapsed == FORMAT_STARDUST_COLLAPSED) {
    ...
    public function section_format_options($foreditform = false) {
        return array(
            ...
            'collapsed' => array(
                ...
                'element_attributes' => array(
                    array(
                        FORMAT_STARDUST_EXPANDED => new lang_string('showexpanded', 'format_stardust'),
                        FORMAT_STARDUST_COLLAPSED => new lang_string('showcollapsed', 'format_stardust'),
                    )
                ),
            ...
            'pinned' => array(
                ...
                'element_attributes' => array(
                    array(
                        FORMAT_STARDUST_UNPINNED => new lang_string('showunpinned', 'format_stardust'),
                        FORMAT_STARDUST_PINNED => new lang_string('showpinned', 'format_stardust'),
                    )
                ),
                'default' => COURSE_DISPLAY_SINGLEPAGE,
            ),
     ...
     *
     * format_stardust processes additional attributes in the view course URL
    ...
    public function page_set_course(moodle_page $page) {
        ...
        if ($this->on_course_view_page()) {
            ...
            if ($switchcollapsed && confirm_sesskey() && has_capability('moodle/course:update', $context)
                ...
                if ($section->collapsed == FORMAT_STARDUST_EXPANDED) {
                    $newvalue = FORMAT_STARDUST_COLLAPSED;
                } else {
                    $newvalue = FORMAT_STARDUST_EXPANDED;
                }
                $this->update_section_format_options(array('id' => $section->id, 'collapsed' => $newvalue));
                if ($newvalue == FORMAT_STARDUST_COLLAPSED) {
                    if (!isset($options['sr'])) {
            ...
            if ($switchpinned && confirm_sesskey() && has_capability('moodle/course:update', $context)
                ...
                if ($newvalue == FORMAT_STARDUST_PINNED) {
                    if (!isset($options['sr'])) {
                        $options['sr'] = $this->find_collapsed_parent($section->parent);
                    }
     ...
     *
     * @param int|section_info $section
     * @return array of format_stardust_edit_control
     */
    public function get_section_edit_controls($section) {
        ...
        if ($sectionnum && has_capability('moodle/course:update', $context) && $sectionnum != $sr) {
            ...
            if ($section->collapsed == FORMAT_STARDUST_EXPANDED) {
                $text = new lang_string('showcollapsed', 'format_stardust');
                $class = 'expanded';
            } else {
                $text = new lang_string('showexpanded', 'format_stardust');
                $class = 'collapsed';
            }
            $controls[] = new format_stardust_edit_control($class, $switchcollapsedurl, $text);
        }
        ...
        if ($sectionnum && has_capability('moodle/course:setcurrentsection', $context)) {
            ...
            if ($course->marker == $section->section) {
                $marker = 0;
                $text = new lang_string('removemarker', 'format_stardust');
                $class = 'marked';
            } else {
                $marker = $section->section;
                $text = new lang_string('setmarker', 'format_stardust');
                $class = 'marker';
            }
            $setmarkerurl->params(array('marker' => $marker, 'sesskey' => sesskey()));
            $controls[] = new format_stardust_edit_control($class, $setmarkerurl, $text);
        }
        ...
        if ($sectionnum && has_capability('moodle/course:update', $context)) {
            ...
            if ($section->pinned == FORMAT_STARDUST_UNPINNED) {
                $text = new lang_string('showpinned', 'format_stardust');
                $class = 'unpinned';
            } else {
                $text = new lang_string('showunpinned', 'format_stardust');
                $class = 'pinned';
            }
            $controls[] = new format_stardust_edit_control($class, $switchpinnedurl, $text);
        }
        ...
        if (has_capability('moodle/course:update', $context)) {
            ...
            $controls[] = new format_stardust_edit_control('settings', $editurl, $text);
        }
        ...
        if ($sectionnum && has_capability('moodle/course:update', $context)) {
            ...
            $text = new lang_string('mergeup', 'format_stardust');
            $controls[] = new format_stardust_edit_control('mergeup', $mergeupurl, $text);
        }
        ...
        if ($sectionnum && has_capability('moodle/course:update', $context)) {
            ...
            $text = new lang_string('deletesection', 'format_stardust');
            $controls[] = new format_stardust_edit_control('delete', $deleteurl, $text);
        }
    ...
    public function get_edit_control_movehere($parent, $before, $sr = null) {
        ...
        return new format_stardust_edit_control('movehere', $movelink, $str);
    }

    /**
     * Returns a control to exit the section moving mode
     *
     * @return null|format_stardust_edit_control
     */
    public function get_edit_controls_cancelmoving() {
        ...
        if ($movingsection) {
            ...
            $str = strip_tags(get_string('cancelmoving', 'format_stardust', $this->get_section_name($movingsection)));
            $controls[] = new format_stardust_edit_control('cancelmovingsection', $cancelmovingurl, $str);
        }
        ...
        if (ismoving($this->courseid)) {
            ...
            $str = strip_tags(get_string('cancelmoving', 'format_stardust', $USER->activitycopyname));
            $controls[] = new format_stardust_edit_control('cancelmovingactivity', $cancelmovingurl, $str);
        }
        return $controls;
    }
    ...
    public function get_add_section_control($parentsection) {
        ...
        if ($parentsection) {
            $text = new lang_string('addsubsection', 'format_stardust');
        } else {
            $text = new lang_string('addsection', 'format_stardust');
        }
        return new format_stardust_edit_control('addsection', $url, $text);
    }
...
/*        if ($this->get_viewed_section()) {
            ...
            if ($section->parent) {
                ...
                $text = new lang_string('backtosection', 'format_stardust', $this->get_section_name($section->parent));
                $url = $this->get_view_url($section->section, array('sr' => $sr,'parent'=>1));
            } else {
                $sr = 0;
                $text = new lang_string('backtocourse', 'format_stardust', $this->get_course()->fullname);
                $url = $this->get_view_url($section->section, array('sr' => $sr));
            }

            return new format_stardust_edit_control('backto', $url, strip_tags($text));
        }
        ...
        if ($PAGE->context && $PAGE->context->contextlevel == CONTEXT_MODULE && $PAGE->cm) {
            ...
            if ($sectionnum) {
                $text = new lang_string('backtosection', 'format_stardust', $this->get_section_name($sectionnum));
            } else {
                $text = new lang_string('backtocourse', 'format_stardust', $this->get_course()->fullname);
            }
...
*/
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_stardust');
        }
        if (empty($editlabel)) {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_stardust_edit_control implements renderable {
    public $url;
    public $text;
    public $class;
 ...
 * @return \core\output\inplace_editable
 */
function format_stardust_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            array($itemid, 'stardust'), MUST_EXIST);
 ...
 * @return bool
 */
function format_stardust_check_params() {
 ...
 */
function format_stardust_get_fontawesome_icon_map() {
    return [
        'format_stardust:mergeup' => 'fa-level-up',
    ];
}
